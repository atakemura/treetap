...
# get leaf index by lgb.predict(predict_leaf_index=True)
# given leaf index, path to the leaf can be extracted
# locally consistent explanation for gradient boosting - how? the log scores...
# shap is influence of each features, attribution method, attribution methods are different class
# to make it locally consistent - assign the class using the classifier?
# decision path extraction is not supported by lightgbm - PR still open
# https://github.com/microsoft/LightGBM/issues/2302
# decision path extraction is available for randomforest and other decision tree algorithms in sklearn
...
def run_experiment_2(dataset_name):

    exp_dir = './tmp/local_explanation'
