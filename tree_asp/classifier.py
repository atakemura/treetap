...
class RuleClassifier:
    ...
    def predict_index(self, X: pd.DataFrame, **params):
        Xdd = dd.from_pandas(X, npartitions=32)
        predicted_ = Xdd.apply(self.predict_single_row_cov, axis=1, meta=(None, np.bool)).compute(scheduler='processes')
        predicted_idx = predicted_.index[predicted_].to_numpy()
    ...
    def predict_single_row_cov(self, row):
